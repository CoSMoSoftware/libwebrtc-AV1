...
To make sure that codec AV1 will be selected by peerconnection_client, patch 0001_av1_svc_sfm_poc.patch modifies the list of video codec that are exchanged during negotiation. The modified list contains only one codec: AV1.
...
Executables `peerconnection_client(.exe)` is located in directory `out/release`